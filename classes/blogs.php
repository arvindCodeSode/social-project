...
class blogs extends dbcon{
        ...
        public function show_data($last_id=null){
            ...
            $last_id=500;
            $html = "";
            $sql  = "SELECT U.`fname`, U.`uname`, P.*,Uimg.*, SUM(likes) AS likes FROM `user` U RIGHT JOIN `post_data` P ON U.user_id=P.user_Id LEFT JOIN `post_like` PL ON P.post_id=PL.post_id LEFT JOIN `user_image` Uimg ON Uimg.user_id=U.user_id WHERE P.post_id < ? GROUP BY P.post_id ORDER BY P.post_id DESC LIMIT 3";
            $stmt=$this->connection->prepare($sql);
